...
def load_elmo(path, max_len=200):
    ...
    with open(path, 'rb') as inf:
        for line in inf:
            ...
            gzip_label = gzip_fields[2]
            elmo_embd_str = gzip_fields[5].strip()
            #print(elmo_embd_str)
            #print(type(elmo_embd_str))
            #lis = (elmo_embd_str.replace("NaN", "0"))
            #print(lis)
            elmo_embd_list = ast.literal_eval(elmo_embd_str)
            #print(len(elmo_embd_list))
            elmo_embd_array = np.array(elmo_embd_list)
            #print(gzip_id,gzip_label)
            #print(elmo_embd_str,len(elmo_embd_str))
            #print(elmo_embd_array,elmo_embd_array.shape)
