...
NRC_path1 = "psycholinguistic_extractors/meta_features_data/NRC-Emotion-Lexicon.xlsx"
...
if __name__ == '__main__':
    ...
    with open(outfile, "wt", encoding="utf8") as outp:
        ...
        for index in range(0,len(df.index)):
            ...
            val1 = lambda index : 'true' if df['type'][index][0]=='E' else 'false'
            val2 = lambda index : 'true' if df['type'][index][1]=='N' else 'false'
            val3 = lambda index : 'true' if df['type'][index][2]=='T' else 'false'
            val4 = lambda index : 'true' if df['type'][index][3]=='J' else 'false'

            fields.append(index)
            fields.append(val1(index))
            fields.append(val2(index))
            fields.append(val3(index))
            fields.append(val4(index))
            ...
            print('writing')
            print(fields[0], fields[1], fields[2], fields[3],fields[4], json.dumps(outs), sep="\t", file=outp)
            nlines += 1
            if nlines % every == 0:
